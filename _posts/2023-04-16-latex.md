--- 
layout: post
title: Latex and Code Presentation
author: Internet
---
...
f_{Y} (x) 
    &=  \sum_{k=0}^{N m - N}
    \frac{ \chi(N, k) }{ \Gamma(N m-k) } 
    \frac{x^{N m-k-1}}{\alpha^{N m-k}} e^{-\frac{x}{\alpha}},~x > 0,
\end{align}

\begin{align}
	N_{\max} = \bigg\lfloor \frac{\mu_1^2}{\mu_2-\mu_1^2} \bigg\rfloor.
\end{align}
